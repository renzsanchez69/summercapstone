    ...
    const requestsFirebaseObject = this.props.doUseFirebaseObject
      ...
      .on("value", snapshot => {
        if (snapshot.exists()) {
          ...
          const updatedRequestWithKey = JSON.parse(
            JSON.stringify(snapshot.val())
          );
          const initAllRequests = [];
          Object.keys(updatedRequestWithKey).forEach(reqKey => {
            initAllRequests.push(updatedRequestWithKey[reqKey]);
          });
          this.setState({ allRequests: initAllRequests, loadingData: false });
        } else {
  ...
  deleteRequest = request => {
    if (request.status == Constants.BOOKING_STATUS.BOOKED) {
      this.props.doSendAReportMessage(
        "Sorry, refer to our no cancellation policy"
      );
      setTimeout(() => {
        this.props.doSendAReportMessage("");
