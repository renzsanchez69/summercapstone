...
export default class Main extends Component {
          ...
          firebase
            ...
            .once("value", snapshot => {
              if (snapshot.exists()) {
                ...
                this.setState({
                  loadingText: "Getting your registration, please wait.."
                });
                this.changeMainApplicationDisplay(
                  Constants.APP_PAGES.LOADING_SCREEN_APP
                );
                ...
                userKey
                  ...
                  .catch(error => {
                    this.sendAReportMessage(
                      "Check your connectivity, you are offline"
                    );
                    setTimeout(() => {
                      this.sendAReportMessage("");
                      this.setState({ loadingText: "" });
              ...
              if (snapshot.exists()) {
                ...
                firebase
                  ...
                  .once("value", snapshot => {
                    if (snapshot.exists()) {
                      this.sendAReportMessage(
                        "The input email is already taken"
                      );
                      setTimeout(
                        () => this.sendAReportMessage(""),
                        Constants.REPORT_DISPLAY_TIME
                      ...
                      firebase
                        ...
                        .once("value", snapshot => {
                          if (snapshot.exists()) {
                            ...
                            this.setState({
                              loadingText:
                                "Getting your registration, please wait.."
                            });
                            this.changeMainApplicationDisplay(
                              Constants.APP_PAGES.LOADING_SCREEN_APP
                            );
  ...
  storeCredentialsLocally = finalAccountInfo => {
    SyncStorage.set(
      Constants.LOCAL_ACCOUNT_KEY,
      JSON.stringify(finalAccountInfo)
    )
      .then(() => {
        ...
        if (snapshot.exists()) {
          ...
          if (String(finalAccountInfo.password) == String(loginData.password)) {
            ...
            setTimeout(() => {
              this.sendAReportMessage("");
              this.changeMainApplicationDisplay(
                Constants.APP_PAGES.HOME_DASHBOARD
              );
            }, Constants.REPORT_DISPLAY_TIME);
          } else {
            console.log("Users!");
          ...
          firebase
            ...
            .once("value", snapshot => {
              if (snapshot.exists()) {
                let accountInfoWithKey = JSON.parse(
                ...
                if (
                  String(finalAccountInfo.password) ==
                  String(loginData.password)
                ) {
                  this.setState({
                    loadingText: Constants.LOADING_TEXT.LOGGING_IN
                  });
                  this.sendAReportMessage(
  ...
  submitRestaurantOwnersLocation = location => {
    ...
    firebase
      .database()
      .ref(
        "RESTAURANT/" + String(this.state.loggedInformation.key) + "/location"
      )
      .update({
      ...
      .then(() => {
        firebase
          ...
          .once("value", snapshot => {
            if (snapshot.exists()) {
              const ownersAccountInformation = JSON.parse(
                JSON.stringify(snapshot.val())
              );
          ...
          .then(() => {
            ...
            this.sendAReportMessage(
              "Successfully updated your restaurant location"
            );
            setTimeout(
              () => this.sendAReportMessage(""),
              Constants.REPORT_DISPLAY_TIME
  ...
  userChangePassword = newPassword => {
    ...
    if (this.state.loggedInformation.role == Constants.ROLES.USER_ONLY) {
      firebase
        ...
        .then(() => {
          firebase
            ...
            .once("value", snapshot => {
              if (snapshot.exists()) {
                let updatedAccountInformation = JSON.parse(
                  JSON.stringify(snapshot.val())
                );
                this.storeCredentialsLocally(updatedAccountInformation);
                this.setState({ loggedInformation: updatedAccountInformation });
        ...
        .catch(error => {
          setTimeout(() => {
            ...
            this.sendAReportMessage(
              "An error occured in upadting your password"
            );
            setTimeout(
              () => this.sendAReportMessage(""),
              Constants.REPORT_DISPLAY_TIME
      ...
      this.state.loggedInformation == Constants.ROLES.RESTAURANT_OWNER
    ) {
      firebase
        .database()
        .ref("RESTAURANT/" + String(this.state.loggedInformation.key))
  ...
  updateUserInformation = data => {
    ...
    if (this.state.loggedInformation.role == Constants.ROLES.USER_ONLY) {
      firebase
        ...
        .catch(error => {
          setTimeout(() => {
            ...
            this.sendAReportMessage(
              "An error occured in upadting your details"
            );
            setTimeout(
              () => this.sendAReportMessage(""),
              Constants.REPORT_DISPLAY_TIME
      ...
      this.state.loggedInformation.role == Constants.ROLES.RESTAURANT_OWNER
    ) {
      firebase
        .database()
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
        {
          ...
          message: "AsaTaKaon application needs access to your location",
  ...
  controlSplashScreen = () => {
    setTimeout(() => {
      ...
      if (this.state.onLoggedIn == true) {
        ...
        this.setState({
          applicationPages: Constants.APP_PAGES.WELCOME_APP_PAGE
        });
      }
  ...
  MainApplicationDisplay = () => {
    switch (this.state.applicationPages) {
      case Constants.APP_PAGES.LOADING_SCREEN_APP:
        return <LoadingScreen loadingText={this.state.loadingText} />;
  ...
  reportApplicationDisplay = () => {
    if (this.state.reportMessage.length != 0) {
      return <ReportDisplay reportMessage={this.state.reportMessage} />;
    } else {
      return;
    }
  };
